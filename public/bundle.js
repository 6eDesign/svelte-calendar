var app = (function () {
  ...
  function keyCodesArray(ref) {
  	var keyCodes = ref.keyCodes;

  	return Object.keys(keyCodes).map(function (k) { return keyCodes[k]; });
  }
  ...
  var methods$2 = { 
    ...
    incrementMonth: function incrementMonth(direction,date) {
      ...
      if(!date) { return this.set({month: month,year: year}); }
    ...
    handleKeyPress: function handleKeyPress(evt) { 
      var ref = this.get();
      var keyCodes = ref.keyCodes;
      var keyCodesArray = ref.keyCodesArray; 
  ...
  function Datepicker(options) {
  	...
  	this._recompute({ start: 1, end: 1, month: 1, year: 1, months: 1, monthIndex: 1, visibleMonth: 1, selected: 1, format: 1, keyCodes: 1 }, this._state);
  	...
  	if (!('keyCodes' in this._state)) { console.warn("<Datepicker> was created without expected data property 'keyCodes'"); }
  ...
  Datepicker.prototype._checkReadOnly = function _checkReadOnly(newState) {
  	...
  	if ('keyCodesArray' in newState && !this._updatingReadonlyProperty) { throw new Error("<Datepicker>: Cannot set read-only property 'keyCodesArray'"); }
  };
  ...
  Datepicker.prototype._recompute = function _recompute(changed, state) {
  	...
  	if (changed.keyCodes) {
  		if (this._differs(state.keyCodesArray, (state.keyCodesArray = keyCodesArray(state)))) { changed.keyCodesArray = true; }
  	}
  };

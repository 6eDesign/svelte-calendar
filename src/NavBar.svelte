<script>
  ...
  $: {
    ...
    availableYears = calendarYears.map((m, i) => {
      return Object.assign({}, {
        label: m
      }, {
        selectable:
          (!isOnLowerBoundary && !isOnUpperBoundary)
          || (
            (!isOnLowerBoundary || i >= start.getFullYear())
            && (!isOnUpperBoundary || i <= end.getFullYear())
          )
      });
    });
...
<div class="title">
  ...
  <div class="year-selector" class:open={yearSelectorOpen}>
    {#each availableYears.reverse() as yearDefinition, index}
      <div
        class="month-selector--month"
        class:selected={yearDefinition.label === year}
        class:selectable={yearDefinition.selectable}
        on:click={e => yearSelected(e, { y: yearDefinition, i: index })}
      >
        <span>{yearDefinition.label}</span>
      </div>
    {/each}
  </div>
</div>
